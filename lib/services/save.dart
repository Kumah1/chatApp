...
class Save {
  ...
  static Future<String> getBase64FromImage({String? imageUrl, File? file}) {
    ...
    if (file == null) {
      DefaultCacheManager()
          ...
          .then((stream) => stream.file.readAsBytes().then((imageBytes) {
  ...
  static void saveToDisk(ImageProvider provider, String filename) async {
    ...
    Directory dir = Directory(appDocDirectory!.path + '/KumChat/photos');
    ...
    if (provider is CachedNetworkImageProvider) {
      ...
      DefaultCacheManager().downloadFile(_cache.url).then((stream) {
